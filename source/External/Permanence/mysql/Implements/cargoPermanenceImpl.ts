...
class CargoPermanenceImpl implements CargoPermanence {
  ...
  constructor(ownerPermanence: OwnerPermanence, truckerPermanence: TruckerPermanence) {
  ...
  changeCargoPacketToCargo(cargoPacket: CargoPacket) {
    const cargo = new Cargo(cargoPacket.CARGO_ID, cargoPacket.NAME, cargoPacket.TRANSPORT_DUE_DATE, cargoPacket.DESCRIPTION, cargoPacket.OWNER_ID, cargoPacket.TRUCKER_ID, cargoPacket.STATUS)
    return cargo
  }
    ...
    async getNewCargoId() {
        ...
        const conn = await createConnection(DB_CONFIG)
        const [rows, fields] : [MaxCargoIdPacket[], FieldPacket[]] = await conn.query(query)
        await conn.end()
    ...
    async saveCargo(cargo: Cargo) {
        ...
        const conn = await createConnection(DB_CONFIG)
        const [rows, fields] : [CargoPacket[], FieldPacket[]] = await conn.query(query)
        await conn.end()
    }
